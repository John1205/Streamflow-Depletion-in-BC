...
stream = pd.read_csv(os.path.join(file_path, "Drain_SFD.csv"), header = 0)
river = pd.read_csv(os.path.join(file_path, "River_SFD.csv"), header = 0)

stream['years'] = stream.periods/52
river['years'] = river.periods/52
...
week_dates['periods'] = week_dates.index+1
week_dates['pump'] = pump
year = np.repeat(np.arange(1, 31), 52)
week_dates['pump_year'] = year

stream_pump = pd.merge(left=stream, right=week_dates, how='left', left_on='periods', right_on='periods')
river_pump = pd.merge(left=river, right=week_dates, how='left', left_on='periods', right_on='periods')
...
stream_sum = stream_sum.drop(columns = ['years', 'days', 'stress','segment','periods', 'pump', 'Q'])

river_pump = river_pump.groupby(['pump_year']).sum().reset_index()
river_pump = river_pump.drop(columns = ['years', 'days', 'stress','segment','periods', 'pump', 'Q'])


ryear10 = river_pump[river_pump['pump_year']==9]
ryear20 = river_pump[river_pump['pump_year']==11]
ryear30 = river_pump[river_pump['pump_year']==12]

syear10 = stream_sum[stream_sum['pump_year']==9]
syear20 = stream_sum[stream_sum['pump_year']==11]
syear30 = stream_sum[stream_sum['pump_year']==12]
...
well_input0 = pd.read_csv(os.path.join(file_path, "well_T_kh.csv"))
...
SFD10=[]
for i in range(2, len(ryear10.columns)):
...
for i in range(2, len(syear10.columns)):
        no = int(syear10.columns[i][colname:]) # change number 
...
#####################################################
SFD20=[]
for i in range(2, len(ryear20.columns)):
...
for i in range(2, len(syear20.columns)):
        no =int(syear20.columns[i][colname:]) # change number 
...
#####################################################
SFD30=[]
for i in range(2, len(ryear30.columns)):
...
for i in range(2, len(syear30.columns)):
        no = int(syear30.columns[i][colname:]) # change number 
...
plt.yticks([])
plt.xticks([])
ax4.set_title('Year 15')
plt.savefig(os.path.join(store_path, "peace_baseflow_yr15.jpg"), dpi = 1000)  
#plt.subplots_adjust(right=0.6)
#cax = plt.axes([0.62, 0.13, 0.015, 0.75])
#plt.colorbar(cax=cax)
#plt.savefig(os.path.join(store_path, "peace_baseflow_yr15.pdf"))  


#############################################################
"""plot MAE for wells"""
mae_file = 'D:\\Peace_GW_2_data'
os.chdir(mae_file)
well_input0 = pd.read_csv(os.path.join(mae_file, "well_T_kh.csv"))
well = pd.read_csv(os.path.join(mae_file, "glover_hunt_wells.csv"), index_col = 0)
...
#plt.scatter(glover_x, glover_y, c= glover_mae, s = glover_s, marker = 'o', cmap = camp,  vmin=vmin, vmax=vmax, norm = norm)
plt.colorbar()
plt.xticks([])
plt.yticks([])
plt.savefig(os.path.join(mae_file, "Hunt_MAE_spatial.jpg"), dpi= 400)
#plt.savefig(os.path.join(mae_file, "Glover_MAE_spatial.jpg"), dpi= 400)
